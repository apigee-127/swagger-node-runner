        ...
        context._finish = function finishConnect(ignore1, ignore2) { // must have arity of 2
          ...
          try {
            ...
            if (context.headers) {
              ...
              // set content type of json as default if supported
              if (!response.getHeader('content-type') &&
                  request.headers['accept'] && request.headers['accept'].match(/json/) &&
            ...
            if (context.output) {
              ...
              if (typeof body === 'object' && response.getHeader('content-type').match(/json/))  {
...
function produces(operation, mimetype) {
  return (operation.pathObject.produces && operation.pathObject.produces.indexOf(mimetype) > -1) ||
         (operation.api.produces && operation.api.produces.indexOf(mimetype) > -1);
  }